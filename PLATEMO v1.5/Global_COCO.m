...
%--------------------------------------------------------------------------
    ...
    methods(Access = private, Static)
        ...
        function save(obj)
            clc; fprintf('%s on %s, %d objectives, %d dimensions, run %d (%6.2f%%), %.2fs passed...\n',...
                         func2str(obj.algorithm),func2str(obj.problem),obj.M,obj.D,obj.run,obj.evaluated/obj.evaluation*100,obj.runtime);
            if obj.evaluated >= obj.evaluation
                ...
                save(fullfile(folder,sprintf('%s_%s_M%d_D%d_%d.mat',func2str(obj.algorithm),func2str(obj.problem),obj.M,obj.D,obj.run)),'Complete', 'Population','PF','metric');
            end
        end
