function Data=data_wfg2_NSGAIII
...
%--------------------------------------------------------------------------
%
Data.xlow =[0.01, 0.01, 0.01, 1];  % variable lower bounds
Data.xup=[0.99, 10, 10, 10];     % variable upper bounds
Data.dim=4;             %problem dimension
Data.integer=[4];        %indices of variables with integer constraints
...
for r = 1:30
    ...
    % load it in
    if exist(strcat(algorithm, '_new_WFG2', '_probindex', pindex, '_', num2str(r), '.mat')) == 2
        load(strcat(algorithm, '_new_WFG2', '_probindex', pindex, '_', num2str(r), '.mat'))
    else
        % Run the algorithm on the problem to get the performance metrics
        main('-algorithm',str2func(algorithm),'-problem',@new_WFG2,'-N',100, '-zmin', x(1), '-pfp1', x(2), '-pfp2', x(3), '-pfp3', x(4), '-run', r, ...
        ...
        load(fullfile(pwd,'Data', 'new_WFG2', algorithm, ...
        strcat(algorithm, '_', 'new_WFG2', '_probindex', pindex_2, '_', num2str(r), '.mat')));
    end
