function Data=data_wfg5_NSGAIII
...
%--------------------------------------------------------------------------
%
Data.xlow =[0.01, 0.01, 0.01];  % variable lower bounds
Data.xup=[0.99, 0.1, 0.1];     % variable upper bounds
...
Data.constraint{1}=@(x) x(1) - x(2);
Data.constraint{2}=@(x) 1-x(1)-x(2);
end %function
...
algnum = 6;
x=x(:)';
% re_s_decept re_s_decept re_s_decept
% x(3) is zmin, x(2) is p2, x(1) is p1
new_x1 = sprintf('%0.4f', x(1));
new_x2 = sprintf('%1.3f', x(2));
new_x3 = sprintf('%1.3f', x(3));
...
for r = 1:30
    ...
    % load it in
    if exist(strcat(algorithm, '_new_WFG5_', 'probindex', pindex, '_', num2str(r), '.mat')) == 2
        load(strcat(algorithm, '_new_WFG5_', 'probindex', pindex, '_', num2str(r), '.mat'))
    else
        % Run the algorithm on the problem to get the performance metrics
        main('-algorithm',str2func(algorithm),'-problem',@new_WFG5,'-N',100, '-zmin', x(1), '-p1', x(2), '-p2', x(3),...,
        ...
        load(fullfile(pwd,'Data', 'new_WFG5', algorithm, ...
        strcat(algorithm, '_', 'new_WFG5_', 'probindex', pindex_2, '_', num2str(r), '.mat')));
    end
