...
%--------------------------------------------------------------------------
%
Data.xlow =[0.01];  % variable lower bounds
Data.xup=[0.99];     % variable upper bounds
...
for r = 1:30
    ...
    % load it in
    if exist(strcat(algorithm, '_new_WFG3', '_probindex', pindex, '_', num2str(r), '.mat')) == 2
        load(strcat(algorithm, '_new_WFG3', '_probindex', pindex, '_', num2str(r), '.mat'))
	elseif probindex > 30 & exist(strcat(algorithm, '_new_WFG3', '_probindex', pindex_2, '_', num2str(r), '.mat')) == 2
		load(strcat(algorithm, '_new_WFG3', '_probindex', pindex_2, '_', num2str(r), '.mat'))
    else
        % Run the algorithm on the problem to get the performance metrics
        main('-algorithm',str2func(algorithm),'-problem',@new_WFG3,'-N',100, '-zmin', x(1), '-run', r, ... 
		'-probindex', probindex + algnum/10, '-save', 1);
        ...
        load(fullfile(pwd,'Data', 'new_WFG3', algorithm, ...
        strcat(algorithm, '_', 'new_WFG3', '_probindex', pindex_2, '_', num2str(r), '.mat')));
    end
