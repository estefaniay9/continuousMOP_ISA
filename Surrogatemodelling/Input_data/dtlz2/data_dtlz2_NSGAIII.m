function Data=data_dtlz2_NSGAIII
...
for r = 1:30
    ...
    % load it in
    if exist(strcat(algorithm, '_new_DTLZ2', '_probindex', pindex, '_', num2str(r), '.mat')) == 2
        load(strcat(algorithm, '_new_DTLZ2', '_probindex', pindex, '_', num2str(r), '.mat'))
    else
        % Run the algorithm on the problem to get the performance metrics
        main('-algorithm',str2func(algorithm),'-problem',@new_DTLZ2,'-N',100, '-p1', x(1), ...
        ...
        load(fullfile(pwd,'Data', 'new_DTLZ2', algorithm, ...
        strcat(algorithm, '_', 'new_DTLZ2', '_probindex', pindex_2, '_', num2str(r), '.mat')));
    end
