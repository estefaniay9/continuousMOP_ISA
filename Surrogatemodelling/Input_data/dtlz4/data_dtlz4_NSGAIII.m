function Data=data_dtlz4_NSGAIII
...
Data.xup=[100, 1];     % variable upper bounds
...
for r = 1:30
    ...
    % load it in
    if exist(strcat(algorithm, '_new_DTLZ4', '_probindex', pindex, '_', num2str(r), '.mat')) == 2
        load(strcat(algorithm, '_new_DTLZ4', '_probindex', pindex, '_', num2str(r), '.mat'))
    else
        % Run the algorithm on the problem to get the performance metrics
        main('-algorithm',str2func(algorithm),'-problem',@new_DTLZ4,'-N',100, '-p1', x(1), '-p2', x(2), ...
        ...
        load(fullfile(pwd,'Data', 'new_DTLZ4', algorithm, ...
        strcat(algorithm, '_', 'new_DTLZ4', '_probindex', pindex_2, '_', num2str(r), '.mat')));
    end
