...
%--------------------------------------------------------------------------
%
Data.xlow =[1, 0, 0.01];  % variable lower bounds
Data.xup=[100, 100, 0.99];     % variable upper bounds
...
Data.constraint{1}=@(x) (4*x(1)+2)*pi-4*x(2);
end %function
...
for r = 1:30
    ...
    if exist(strcat(algorithm, '_new_WFG4', '_probindex', pindex, '_', num2str(r), '.mat')) == 2
        ...
        main('-algorithm',str2func(algorithm),'-problem',@new_WFG4,'-N',100, '-zmin', x(3), '-p1', x(1), '-p2', x(2), ...
