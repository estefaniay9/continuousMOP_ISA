...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 2 : 2 : M
                for j = 1 : obj.nk
                    G(:,i) = G(:,i) + Deceptive4(PopDec(:,obj.len(i)+M-1+(j-1)*obj.sublen(i)+1:obj.len(i)+M-1+j*obj.sublen(i)));
                end
            end
...
function f = Deceptive4(x) %biased deceptive
	f = 2*(10 - 10*exp(-0.2*sqrt((1/2).*sum((x-1).^2,2)))).^0.1;
end
