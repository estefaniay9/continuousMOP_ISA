classdef LSMOP6_DEC65_2_10 < PROBLEM
% <problem> <LSMOP>
...
%--------------------------------------------------------------------------
    ...
    methods
        %% Initialization
        function obj = LSMOP6_DEC65_2_10()
            % Parameter setting
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i = 1 : 2 : M
                for j = 1 : obj.nk
                    G(:,i) = G(:,i) + Deceptive6(PopDec(:,obj.len(i)+M-1+(j-1)*obj.sublen(i)+1:obj.len(i)+M-1+j*obj.sublen(i)));
                end
            end
            for i = 2 : 2 : M
                for j = 1 : obj.nk
...
function f = Deceptive5(x) %biased deceptive
	f = 10*(10 -10*exp(-sqrt((1/2).*sum((x-1).^2,2)))-sum(sin(x-1).^2,2)).^0.01;
end

function f = Deceptive6(x) %biased and deceptive
	f = 2*(2*sqrt(abs(sum(x-x.^2,2)))+0.01*abs(sum(x.^2,2))).^0.01;
end
